                                ...
                                <div class="pagination d-flex justify-content-center mt-5">
                                    <%
                                        if(Integer.parseInt(request.getAttribute("page").toString()) > 1){
                                            out.print("<a href=\"/search?keyWord=" + request.getAttribute("keyWord")
                                                    + "&page=" + (Integer.parseInt(request.getAttribute("page").toString()) - 1) + "\" class=\"rounded\">&laquo;</a>");
                                        }
                                        if(Integer.parseInt(request.getAttribute("page").toString()) == 1){
                                            out.print("<a href=\"/search?keyWord=" + request.getAttribute("keyWord")
                                                    + "&page=" + 1 + "\" class=\"rounded\">&laquo;</a>");
                                        }
                                        for(int i = 0; i < Integer.parseInt(request.getAttribute("fullPage").toString()); i++){
                                            if(i + 1 == Integer.parseInt(request.getAttribute("page").toString())){
                                                out.print("<a href=\"/search?keyWord=" + request.getAttribute("keyWord")
                                                        + "&page=" + (i + 1) + "\" class=\"active rounded\">" + (i + 1) + "</a>");
                                            }else{
                                                out.print("<a href=\"/search?keyWord=" + request.getAttribute("keyWord")
                                                        + "&page=" + (i + 1) + "\" class=\"rounded\">" + (i + 1) + "</a>");
                                            }
                                        }
                                        if(Integer.parseInt(request.getAttribute("page").toString()) < Integer.parseInt(request.getAttribute("fullPage").toString())){
                                            out.print("<a href=\"/search?keyWord=" + request.getAttribute("keyWord")
                                                    + "&page=" + (Integer.parseInt(request.getAttribute("page").toString()) + 1) + "\" class=\"rounded\">&raquo;</a>");
                                        }
                                        if(Integer.parseInt(request.getAttribute("page").toString()) == Integer.parseInt(request.getAttribute("fullPage").toString())){
                                            out.print("<a href=\"/search?keyWord=" + request.getAttribute("keyWord")
                                                    + "&page=" + Integer.parseInt(request.getAttribute("fullPage").toString()) + "\" class=\"rounded\">&raquo;</a>");
                                        }
                                    %>
                                </div>
                            </div>
